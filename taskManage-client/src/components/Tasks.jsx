...
const Tasks = () => {
    ...
    return (
        <div className="min-h-[370px] grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 rounded-lg">
            {/* todo */}
            <div className="bg-[#eaeadd] rounded-lg shadow-lg px-4 py-5 flex flex-col items-center">
                ...
                <div className="flex flex-col gap-2 w-full">
                    {
                        todos.map(todo =>
                            <div key={todo._id} className="items bg-white dark:bg-gray-800 dark:text-white shadow-lg px-2 py-1 w-full flex flex-row justify-between items-center">
                                <p>{todo.title}</p>
                                <div className="flex items-center gap-3">
                                    <FaEye onClick={() => handleModal(`${todo._id}`)} className="cursor-pointer hover:text-gray-500" />
                                    <FaRegEdit onClick={() => navigate(`/dashboard/update/${todo._id}`)} className="cursor-pointer hover:text-gray-500" />
                                    <AiOutlineDelete onClick={() => handleDelete(`${todo._id}`)} className="cursor-pointer hover:text-gray-500" />
                                    <VscDebugStart onClick={() => handleInProgress(`${todo._id}`, "in-progress")} className="cursor-pointer hover:text-gray-500" />
                                </div>
                            </div>)
                    }
                </div>
                ...
                <div className="flex flex-col gap-2 w-full">
                    {
                        progresses.map(progress =>
                            <div key={progress._id} className="items bg-white dark:bg-gray-800 dark:text-white shadow-lg px-2 py-1 w-full flex flex-row justify-between items-center">
                                <p>{progress.title}</p>
                                <div className="flex items-center gap-3">
                                    <FaEye onClick={() => handleModal(`${progress._id}`)} className="cursor-pointer hover:text-gray-500" />
                                    <FaRegEdit onClick={() => navigate(`/dashboard/update/${progress._id}`)} className="cursor-pointer hover:text-gray-500" />
                                    <AiOutlineDelete onClick={() => handleDelete(`${progress._id}`)} className="cursor-pointer hover:text-gray-500" />
                                    <MdOutlineDoneOutline onClick={() => handleInProgress(`${progress._id}`, "done")} className="cursor-pointer hover:text-gray-500" />
                                </div>
                            </div>)
                    }
                </div>
            ...
            <div className="bg-[#a0bebb] min-h-full rounded-lg shadow-lg px-6 py-5 flex flex-col items-center">
                ...
                <div className="flex flex-col gap-2 w-full">
                    {
                        doneTasks.map(done =>
                            <div key={done._id} className="items bg-white dark:bg-gray-800 dark:text-white shadow-lg px-2 py-1 w-full flex flex-row justify-between items-center">
                                <p>{done.title}</p>
                                <div className="flex items-center gap-3">
                                    <FaEye onClick={() => handleModal(`${done._id}`)} className="cursor-pointer hover:text-gray-500" />
                                    <FaRegEdit onClick={() => navigate(`/dashboard/update/${done._id}`)} className="cursor-pointer hover:text-gray-500" />
                                    <AiOutlineDelete onClick={() => handleDelete(`${done._id}`)} className="cursor-pointer hover:text-gray-500" />
                                </div>
                            </div>)
                    }
                </div>
